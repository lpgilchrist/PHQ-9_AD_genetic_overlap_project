...
exposure_file <- list.files(pattern = "no_APOE")
outcome_file <- list.files(pattern = "PHQ9_|MDD_")
...
for (i in exposure_file) {
    for (j in outcome_file) {
        ...
        exposure_name <- str_remove(i, pattern = ".txt.gz") %>% 
            str_remove(pattern = "_munged_MAF_0.01") %>% 
            str_remove(pattern = "AD_sumstats_")
        outcome_name <- str_remove(j, pattern = "_cont_full_mtag_meta_with_N.txt.gz") %>% 
...
write.table(mr_cause_elpd, file = "/scratch/users/k20113596/Rosalind_Transfer/MR/CAUSE/mr_cause_elpd_no_apoe_reverse.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
write.table(mr_cause_effects, file = "/scratch/users/k20113596/Rosalind_Transfer/MR/CAUSE/mr_cause_effects_no_apoe_reverse.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
write.table(mr_cause_sharing, file = "/scratch/users/k20113596/Rosalind_Transfer/MR/CAUSE/mr_cause_sharing_no_apoe_reverse.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
